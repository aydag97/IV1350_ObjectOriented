...
public class InventorySystem {
     ...
     * Throws an exception if the item ID is not in the inventory system or the hardcoded item ID 4 has to be
     * retrieved from inventory system.
     * 
    ...
    public ItemDTO getItemInfo(int itemID) throws ItemNotFoundException, DatabaseFailureException {
      if(itemID == 4){
        throw new DatabaseFailureException("Something went wrong with server connection. Please try again later!");
      }
      ...
      throw new ItemNotFoundException("Item with item ID " + itemID + " not found!");
    }
